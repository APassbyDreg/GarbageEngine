#define MINIZ_EXPORT
/* miniz.c 2.2.0 - public domain deflate/inflate, zlib-subset, ZIP reading/writing/appending, PNG
   ...
   *pArchive_name, const void *pBuf, size_t buf_size, const void *pComment, mz_uint16 comment_size,
   ...
   directory could be lost with this method if anything goes wrong, though.
     ...
     No spanning support. Extraction functions can only handle unencrypted, stored or deflated
   files. Requires streams capable of seeking.
...
extern "C"
{
    ...
    /* For more compatibility with zlib, miniz.c uses unsigned long for some parameters/struct
     * members. Beware: mz_ulong can be either 32 or 64-bits! */
...
#define MZ_DEFAULT_WINDOW_BITS 15
    ...
    MINIZ_EXPORT int mz_deflateInit2(mz_streamp pStream, int level, int method, int window_bits,
    ...
    /* mz_deflate() compresses the input to output, consuming as much of the input and producing as
    ...
    /* Return values: */
    /*   MZ_OK on success (when flushing, or if more input is needed but not available, and/or
     ...
     * call mz_deflate() on the stream anymore. */
    /*   MZ_STREAM_ERROR if the stream is bogus. */
     ...
     * sized large enough to decompress the entire stream in a single call (this is slightly
     * faster). */
    /*   MZ_FINISH implies that there are no more source bytes available beside what's already in
     * the input buffer, and that the output buffer is large enough to hold the rest of the
     * decompressed data. */
    /* Return values: */
    ...
    /* Returns a string description of the specified error code, or NULL if the error code is
     * invalid. */
...
extern "C"
{
    ...
    extern MINIZ_EXPORT void *miniz_def_realloc_func(void *opaque, void *address, size_t items,
                                                     size_t size);
...
#define TDEFL_LESS_MEMORY 0
    ...
    /* tdefl_init() compression flags logically OR'd together (low 12 bits contain the max. number
     * of probes per dictionary search): */
    /* TDEFL_DEFAULT_MAX_PROBES: The compressor defaults to 128 dictionary probes per dictionary
     * search. 0=Huffman only, 1=Huffman+LZ (fastest/crap compression), 4095=Huffman+LZ
    ...
    /* TDEFL_WRITE_ZLIB_HEADER: If set, the compressor outputs a zlib header before the deflate
     * data, and the Adler-32 of the source data at the end. Otherwise, you'll get raw deflate data.
     */
    /* TDEFL_COMPUTE_ADLER32: Always compute the adler-32 of the input data (even when not writing
     * zlib headers). */
    /* TDEFL_GREEDY_PARSING_FLAG: Set to use faster greedy parsing, instead of more efficient lazy
     * parsing. */
    /* TDEFL_NONDETERMINISTIC_PARSING_FLAG: Enable to decrease the compressor's initialization time
     * to the minimum, but the output may vary from run to run given the same input (depending on
    ...
    /* tdefl_compress_mem_to_heap() compresses a block in memory to a heap block allocated via
     * malloc(). */
    /* On entry: */
    ...
    /*  *pOut_len will be set to the compressed data's size, which could be larger than src_buf_len
     * on uncompressible data. */
    /*  The caller must free() the returned block when it's no longer needed. */
    MINIZ_EXPORT void *tdefl_compress_mem_to_heap(const void *pSrc_buf, size_t src_buf_len,
                                                  size_t *pOut_len, int flags);

    /* tdefl_compress_mem_to_mem() compresses a block in memory to another block in memory. */
    /* Returns 0 on failure. */
    MINIZ_EXPORT size_t tdefl_compress_mem_to_mem(void *pOut_buf, size_t out_buf_len,
    ...
    /*  level may range from [0,10], use MZ_NO_COMPRESSION, MZ_BEST_SPEED, MZ_BEST_COMPRESSION, etc.
     * or a decent default is MZ_DEFAULT_LEVEL */
    ...
    MINIZ_EXPORT void *tdefl_write_image_to_png_file_in_memory_ex(const void *pImage, int w, int h,
                                                                  int num_chans, size_t *pLen_out,
                                                                  mz_uint level, mz_bool flip);
    MINIZ_EXPORT void *tdefl_write_image_to_png_file_in_memory(const void *pImage, int w, int h,
                                                               int num_chans, size_t *pLen_out);
    ...
    /* tdefl_compress_mem_to_output() compresses a block to an output stream. The above helpers use
...
enum
{
     ...
     * flexible enough. The low-level functions don't make any heap allocations, unlike the above
     * helper functions. */
    typedef enum
    {
    ...
    /* There is no corresponding deinit() function because the tdefl API's do not dynamically
     * allocate memory. */
    /* pBut_buf_func: If NULL, output data will be supplied to the specified callback. In this case,
     * the user should call the tdefl_compress_buffer() API for compression. */
    /* If pBut_buf_func is NULL the user should always call the tdefl_compress() API. */
    /* flags: See the above enums (TDEFL_HUFFMAN_ONLY, TDEFL_WRITE_ZLIB_HEADER, etc.) */
    MINIZ_EXPORT tdefl_status tdefl_init(tdefl_compressor *d, tdefl_put_buf_func_ptr pPut_buf_func,
                                         void *pPut_buf_user, int flags);
    ...
    MINIZ_EXPORT tdefl_status tdefl_compress(tdefl_compressor *d, const void *pIn_buf,
    ...
    MINIZ_EXPORT tdefl_status tdefl_compress_buffer(tdefl_compressor *d, const void *pIn_buf,
    ...
    /* level may range from [0,10] (where 10 is absolute max compression, but may be much slower on
     * some files) */
    /* window_bits may be -15 (raw deflate) or 15 (zlib) */
    /* strategy may be either MZ_DEFAULT_STRATEGY, MZ_FILTERED, MZ_HUFFMAN_ONLY, MZ_RLE, or MZ_FIXED
     */
    MINIZ_EXPORT mz_uint tdefl_create_comp_flags_from_zip_params(int level, int window_bits,
                                                                 int strategy);
...
extern "C"
{
    ...
    /* TINFL_FLAG_PARSE_ZLIB_HEADER: If set, the input has a valid zlib header and ends with an
     * adler32 checksum (it's a valid zlib stream). Otherwise, the input is a raw deflate stream. */
     ...
     * the entire decompressed stream. If clear, the output buffer is at least the size of the
    ...
    /*  *pOut_len will be set to the decompressed data's size, which could be larger than
     * src_buf_len on uncompressible data. */
...
#define TINFL_DECOMPRESS_MEM_TO_MEM_FAILED ((size_t)(-1))
    MINIZ_EXPORT size_t tinfl_decompress_mem_to_mem(void *pOut_buf, size_t out_buf_len,
                                                    const void *pSrc_buf, size_t src_buf_len,
                                                    int flags);
...
#define TINFL_LZ_DICT_SIZE 32768
    ...
    typedef enum
    {
        ...
        /* is probably corrupted. If you call the inflator again with more bytes it'll try to
         * continue processing the input but this is a BAD sign (either the data is corrupted or you
        ...
        /* This flag indicates that one or more of the input parameters was obviously bogus. (You
         * can try calling it again, but if you get this error the calling code is wrong.) */
        TINFL_STATUS_BAD_PARAM = -3,

        /* This flags indicate the inflator is finished but the adler32 check of the uncompressed
         * data didn't match. If you call it again it'll return TINFL_STATUS_DONE. */
        TINFL_STATUS_ADLER32_MISMATCH = -2,

        /* This flags indicate the inflator has somehow failed (bad code, corrupted input, etc.). If
         * you call it again without resetting via tinfl_init() it it'll just keep on returning the
         * same status failure code. */
        TINFL_STATUS_FAILED = -1,

        /* Any status code less than TINFL_STATUS_DONE must indicate a failure. */

        /* This flag indicates the inflator has returned every byte of uncompressed data that it
         * can, has consumed every byte that it needed, has successfully reached the end of the
         * deflate stream, and */
        /* if zlib headers and adler32 checking enabled that it has successfully checked the
         * uncompressed data's adler32. If you call it again you'll just get TINFL_STATUS_DONE over
         * and over again. */
        TINFL_STATUS_DONE = 0,

        /* This flag indicates the inflator MUST have more input data (even 1 byte) before it can
         * make any more forward progress, or you need to clear the TINFL_FLAG_HAS_MORE_INPUT */
        /* flag on the next call if you don't have any more source data. If the source data was
         * somehow corrupted it's also possible (but unlikely) for the inflator to keep on demanding
         * input to */
         ...
         * a lot of uncompressed data to the caller. I've been assuming you know how much
         * uncompressed data to expect */
        /* (either exact or worst case) and will stop calling the inflator and fail after receiving
         * too much. In pure streaming scenarios where you have no idea how many bytes to expect
...
#define tinfl_get_adler32(r) (r)->m_check_adler32
     ...
     * output. */
    MINIZ_EXPORT tinfl_status tinfl_decompress(tinfl_decompressor *r, const mz_uint8 *pIn_buf_next,
...
extern "C"
{
    ...
    enum
    {
        /* Note: These enums can be reduced as needed to save memory or stack space - they are
         * pretty conservative. */
    ...
    typedef struct
    {
        ...
        /* Byte offset of this entry in the archive's central directory. Note we currently only
         * support up to UINT_MAX or less bytes in the central dir. */
        ...
        /* File's uncompressed size. Note, I've seen some old archives where directory entries had
         * 512 bytes for their uncompressed sizes, but when you try to unpack them you actually get
         * 0 bytes. */
        ...
        /* MZ_TRUE if the file is not encrypted, a patch file, and if it uses a compression method
         * we support. */
    ...
    typedef size_t (*mz_file_write_func)(void *pOpaque, mz_uint64 file_ofs, const void *pBuf,
                                         size_t n);
    ...
    typedef enum
    {
        ...
        MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY =
            0x2000, /* validate the local headers, but don't decompress the entire file and check
                       the crc32 */
        MZ_ZIP_FLAG_WRITE_ZIP64 =
            0x4000, /* always use the zip64 file format, instead of the original zip file format
                       with automatic switch to zip64. Use as flags parameter with
    ...
    /* miniz error codes. Be sure to update mz_zip_get_error_string() if you add or modify this
     * enum. */
    typedef enum
    {
        MZ_ZIP_NO_ERROR = 0,
    ...
    MINIZ_EXPORT mz_bool mz_zip_reader_init_mem(mz_zip_archive *pZip, const void *pMem, size_t size,
                                                mz_uint flags);

#ifndef MINIZ_NO_STDIO
    /* Read a archive from a disk file. */
    /* file_start_ofs is the file offset where the archive actually begins, or 0. */
    /* actual_archive_size is the true total size of the archive, which may be smaller than the
     * file's actual size on disk. If zero the entire file is treated as the archive. */
    MINIZ_EXPORT mz_bool mz_zip_reader_init_file(mz_zip_archive *pZip, const char *pFilename,
                                                 mz_uint32 flags);
    MINIZ_EXPORT mz_bool mz_zip_reader_init_file_v2(mz_zip_archive *pZip, const char *pFilename,
    ...
    MINIZ_EXPORT mz_bool mz_zip_reader_init_cfile(mz_zip_archive *pZip, MZ_FILE *pFile,
    ...
    MINIZ_EXPORT size_t mz_zip_read_archive_data(mz_zip_archive *pZip, mz_uint64 file_ofs,
                                                 void *pBuf, size_t n);
    ...
    MINIZ_EXPORT mz_uint mz_zip_reader_get_filename(mz_zip_archive *pZip, mz_uint file_index,
                                                    char *pFilename, mz_uint filename_buf_size);

    /* Attempts to locates a file in the archive's central directory. */
    /* Valid flags: MZ_ZIP_FLAG_CASE_SENSITIVE, MZ_ZIP_FLAG_IGNORE_PATH */
    /* Returns -1 if the file cannot be found. */
    MINIZ_EXPORT int mz_zip_reader_locate_file(mz_zip_archive *pZip, const char *pName,
                                               const char *pComment, mz_uint flags);
    MINIZ_EXPORT mz_bool mz_zip_reader_locate_file_v2(mz_zip_archive *pZip, const char *pName,
                                                      const char *pComment, mz_uint flags,
                                                      mz_uint32 *file_index);

    /* Returns detailed information about an archive file entry. */
    MINIZ_EXPORT mz_bool mz_zip_reader_file_stat(mz_zip_archive *pZip, mz_uint file_index,
    ...
    MINIZ_EXPORT mz_bool mz_zip_reader_extract_to_mem(mz_zip_archive *pZip, mz_uint file_index,
    ...
    MINIZ_EXPORT void *mz_zip_reader_extract_to_heap(mz_zip_archive *pZip, mz_uint file_index,
    ...
    MINIZ_EXPORT mz_zip_reader_extract_iter_state *mz_zip_reader_extract_file_iter_new(
    ...
    MINIZ_EXPORT mz_bool mz_zip_reader_extract_to_file(mz_zip_archive *pZip, mz_uint file_index,
    ...
    MINIZ_EXPORT mz_bool mz_zip_reader_extract_to_cfile(mz_zip_archive *pZip, mz_uint file_index,
...
/* TODO */
    ...
    /* This function compares the archive's local headers, the optional local zip64 extended
     * information block, and the optional descriptor following the compressed data vs. the data in
    ...
    MINIZ_EXPORT mz_bool mz_zip_validate_file(mz_zip_archive *pZip, mz_uint file_index,
                                              mz_uint flags);
    ...
    MINIZ_EXPORT mz_bool mz_zip_validate_mem_archive(const void *pMem, size_t size, mz_uint flags,
                                                     mz_zip_error *pErr);
    MINIZ_EXPORT mz_bool mz_zip_validate_file_archive(const char *pFilename, mz_uint flags,
                                                      mz_zip_error *pErr);
    ...
    MINIZ_EXPORT mz_bool mz_zip_writer_init_file_v2(mz_zip_archive *pZip, const char *pFilename,
                                                    mz_uint64 size_to_reserve_at_beginning,
                                                    mz_uint flags);
    MINIZ_EXPORT mz_bool mz_zip_writer_init_cfile(mz_zip_archive *pZip, MZ_FILE *pFile,
                                                  mz_uint flags);
#endif
    ...
    MINIZ_EXPORT mz_bool mz_zip_writer_add_mem_ex_v2(
        mz_zip_archive *pZip, const char *pArchive_name, const void *pBuf, size_t buf_size,
    ...
    /* File data is supplied via a read callback function. User mz_zip_writer_add_(c)file to add a
     * file directly.*/
    MINIZ_EXPORT mz_bool mz_zip_writer_add_read_buf_callback(
        mz_zip_archive *pZip, const char *pArchive_name, mz_file_read_func read_callback,
     ...
     */
    MINIZ_EXPORT mz_bool mz_zip_writer_add_cfile(mz_zip_archive *pZip, const char *pArchive_name,
    ...
    /* This function fully clones the source file's compressed data (no recompression), along with
     * its full filename, extra data (it may add or modify the zip64 local header extra data field),
     * and the optional descriptor following the compressed data. */
     ...
     * We could then truncate the file (so the old central dir would be at the end) if something
     * goes wrong. */
    MINIZ_EXPORT mz_bool mz_zip_add_mem_to_archive_file_in_place(
        const char *pZip_filename, const char *pArchive_name, const void *pBuf, size_t buf_size,
        const void *pComment, mz_uint16 comment_size, mz_uint level_and_flags);
